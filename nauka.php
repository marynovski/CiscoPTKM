										...
										else
										{
											...
											if($rezultat =@$polaczenie->query($sql))
											{
												...
												if($ile_lekcji>0)
												{
													...
													{
														$sql = "SELECT * FROM lekcje WHERE id='$k'";																													
														
															$result = $polaczenie->query($sql);																														$ile = $result->num_rows;																														if($ile > 0)															{
																$followingdata = $result->fetch_assoc();																$nr = $followingdata['nr_lekcji'];																$temat = $followingdata['temat'];
																echo	'<div class="lessons-div-in-one" style="height: 50px;">';																echo	$nr;																echo	'</div>';
																	
																echo '<div class="lessons-div-in-two" style="height: 50px; text-align: left;">';
																	
																echo	$temat;
																echo '<form method="post" action="lekcja.php">';
																echo '<input type="text" name ="id_lekcji" value="'; echo $i; echo '" style="visibility: hidden" />';
																echo '<input type="submit" value="Do lekcji" />';
																echo '</form>';
